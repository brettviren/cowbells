...
from sim.units import eV, cm, m
from material import make_medium, make_mixture
...
def register(mc):
    ...
    assert mat, 'Failed to make water from %s' % str(parts)
    med = make_medium(mat)
    wid = med.GetId()           # Must. Love. The. Interface.

    nentries = len(energy)
    efficiency = array('d',[0.0]*nentries)
    print 'energy:',energy
    print 'abs:',absorption
    print 'eff:',efficiency
    print 'rind:',rindex
    mc.SetCerenkov(wid, nentries, energy, absorption, efficiency, rindex)
