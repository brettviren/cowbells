...
//   theRayleighScatteringProcess->DumpPhysicsTable();
  ...
  // add scintillation process and saturation (controlled by birk's constant)
  G4Scintillation*      theScintProcess           = new G4Scintillation("Scintillation");
  G4EmSaturation*       theEmSatProcess           = new G4EmSaturation();
  
  theScintProcess->SetTrackSecondariesFirst(true);
  theScintProcess->SetScintillationYieldFactor(1.0);
  theScintProcess->AddSaturation(theEmSatProcess);
  ...
  theCherenkovProcess->SetTrackSecondariesFirst(false);
