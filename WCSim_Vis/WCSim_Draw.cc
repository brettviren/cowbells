...
void WCSim_Draw::DrawPMT(Float_t x,Float_t y,Float_t z,Float_t color,Float_t size,Bool_t isReduced)
{
...
void WCSim_Draw::PrintGammaInfo(Float_t gamma1E, Float_t gamma2E)
{
...
void WCSim_Draw::DrawSizeLegend(Float_t min, Float_t max, Int_t mode)
{
  // mode: 0 = time   (ns)
  //       1 = energy (pE)
...
void WCSim_Draw::DrawSizeLegend(const char *min, const char *max)
{
...
void WCSim_Draw::DrawColorLegend(Float_t min, Float_t max, Int_t mode)
{
  // mode: 0 = energy (pE)
  //       1 = time   (ns)
...
void WCSim_Draw::DrawColorLegend(const char *min, const char *max)
{
...
void WCSim_Draw::Create2TonePalette(Int_t nclrs)
{
...
void WCSim_Draw::CreateMultiColorPalette(Int_t nclrs)
{
...
void WCSim_Draw::DrawBackground(Int_t event)
{
  char label[50];
...
void WCSim_Draw::DrawBackground(const char *title){
  TCanvas *c1 = fEcan->GetCanvas();
  c1->SetFillColor(10);
  empty_hist[0]->Draw();
...
void WCSim_Draw::GetDimensions()
{
  ...
  if (wcsimrootgeom->GetGeo_Type()==0){
    ...
    l1 = new TLine(-wcsimrootgeom->GetMailBox_x()/2.,-(wcsimrootgeom->GetMailBox_z())/2,-wcsimrootgeom->GetMailBox_x()/2.,(wcsimrootgeom->GetMailBox_z())/2);
    l2 = new TLine(wcsimrootgeom->GetMailBox_x()/2.,-(wcsimrootgeom->GetMailBox_z())/2,wcsimrootgeom->GetMailBox_x()/2.,(wcsimrootgeom->GetMailBox_z())/2);
    l3 = new TLine(-wcsimrootgeom->GetMailBox_x()/2.-wcsimrootgeom->GetMailBox_y(),-(wcsimrootgeom->GetMailBox_z())/2,-wcsimrootgeom->GetMailBox_x()/2.-wcsimrootgeom->GetMailBox_y(),(wcsimrootgeom->GetMailBox_z())/2);
    l4 = new TLine(wcsimrootgeom->GetMailBox_x()/2.+wcsimrootgeom->GetMailBox_y(),-(wcsimrootgeom->GetMailBox_z())/2,wcsimrootgeom->GetMailBox_x()/2.+wcsimrootgeom->GetMailBox_y(),(wcsimrootgeom->GetMailBox_z())/2);
...
void WCSim_Draw::DrawVertexLegend(Int_t real,
			    Int_t rec,
			    Int_t gamma1,
			    Int_t gamma2)
{
...
void WCSim_Draw::DrawVertex(Float_t x, Float_t y, Float_t z, Int_t option)
{
...
void WCSim_Draw::DrawSignal(Int_t a){
  if (a==1){
	...
	if (mcevent->GetEnergy(j)>npe_cut && mcevent->GetTime(j)> ltiming_cut && mcevent->GetTime(j)<htiming_cut){
	  if (log_scale==0){
...
void WCSim_Draw::SwitchToSubcanvas(Int_t i)
{
  TCanvas *c1 = fEcan->GetCanvas();
  c1->cd();
...
void WCSim_Draw::DoText(const char *)
{
  ...
  case HId4:
    ...
    break;
  case HId5:
    SetTiming_lcut(atof(fTbh5->GetString()));
    break;
  case HId6:
    SetTiming_hcut(atof(fTbh6->GetString()));
    break;
  default:
    break;
  }
...
void WCSim_Draw::PrintEventNo(){
  cout << eventno << endl;
}

void WCSim_Draw::SetEventNo(Int_t a){
  eventno = a;
...
void WCSim_Draw::DoExit()
{
   printf("Exit application...");
   gApplication->Terminate(0);
}

// void WCSim_Draw::SetStatusText(const char *txt, Int_t pi)
// {
//    // Set text in status bar.
//    // fStatusBar->SetText(txt,pi);
// }

// void WCSim_Draw::EventInfo(Int_t event, Int_t px, Int_t py, TObject *selected)
// {
...
WCSim_Draw::WCSim_Draw(TString filename, const TGWindow *p, UInt_t w, UInt_t h) :
   TGMainFrame(p, w, h)
{
...
//   myc->Connect("ProcessedEvent(Int_t,Int_t,Int_t,TObject*)","WCSim_Draw",this, 
//                "EventInfo(Int_t,Int_t,Int_t,TObject*)");
  ...
  hframe1->AddFrame(fCheckMulti, new TGLayoutHints(kLHintsTop | kLHintsLeft,
					  5, 5, 5, 5));
  fCheckMulti->Connect("Clicked()", "WCSim_Draw", this, "HandleButtons()"); 
  ...
  horizontal->Connect("Pressed(Int_t)", "WCSim_Draw", this, "SetLogType(Int_t)");
  ...
  horizontal1->Connect("Pressed(Int_t)", "WCSim_Draw", this, "SetDrawMode(Int_t)");
  ...
  horizontal2->Connect("Pressed(Int_t)", "WCSim_Draw", this, "SetFlagVR(Int_t)");
  ...
  fTbh2->AddText(0, "0");
  fTeh2->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe1->AddFrame(fTeh2,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));

 
  

  fTeh1 = new TGTextEntry(hframe2, fTbh1 = new TGTextBuffer(3), HId1);
  fTeh1->SetToolTipText("EventNo");
  fTbh1->AddText(0, "0");
  fTeh1->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe2->AddFrame(fTeh1,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));
  
  TGTextButton *draw = new TGTextButton(hframe2, "&Draw");
  draw->Connect("Clicked()", "WCSim_Draw", this, "DoDraw()");
  hframe2->AddFrame(draw, new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));
  
  TGTextButton *prev = new TGTextButton(hframe2, "&Prev ");
  prev->Connect("Clicked()", "WCSim_Draw", this, "DoPrev()");
  hframe2->AddFrame(prev, new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));

  TGTextButton *next = new TGTextButton(hframe2, "&Next ");
  next->Connect("Clicked()", "WCSim_Draw", this, "DoNext()");
  hframe2->AddFrame(next, new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));


  fTeh3 = new TGTextEntry(hframe2, fTbh3 = new TGTextBuffer(2), HId3);
  fTeh3->SetToolTipText("Set No");
  fTbh3->AddText(0, "0");
  fTeh3->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe2->AddFrame(fTeh3,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));

  fTeh4 = new TGTextEntry(hframe1, fTbh4 = new TGTextBuffer(5), HId4);
  fTeh4->SetToolTipText("Refraction of Index");
  fTbh4->AddText(0, "1.333");
  fTeh4->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe1->AddFrame(fTeh4,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));

  fTeh5 = new TGTextEntry(hframe1, fTbh5 = new TGTextBuffer(5), HId5);
  fTeh5->SetToolTipText("Low Timing Cut");
  fTbh5->AddText(0, "0");
  fTeh5->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe1->AddFrame(fTeh5,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));
  
  fTeh6 = new TGTextEntry(hframe1, fTbh6 = new TGTextBuffer(5), HId6);
  fTeh6->SetToolTipText("High Timing Cut");
  fTbh6->AddText(0, "1000");
  fTeh6->Connect("TextChanged(char*)", "WCSim_Draw", this, "DoText(char*)");
  hframe1->AddFrame(fTeh6,new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));


  TGTextButton *save = new TGTextButton(hframe2, "&Save ");
  save->Connect("Pressed()", "WCSim_Draw", this, "DoSave()");
  hframe2->AddFrame(save, new TGLayoutHints(kLHintsCenterX, 5, 5, 3, 4));

  TGTextButton *exit = new TGTextButton(hframe2, "&Exit ");
  exit->Connect("Pressed()", "WCSim_Draw", this, "DoExit()");
  ...
  hframe2->Resize(100, 25);

  AddFrame(hframe1, new TGLayoutHints( kLHintsCenterX | kLHintsExpandX, 2, 2, 2, 2));
  AddFrame(hframe2, new TGLayoutHints( kLHintsCenterX | kLHintsExpandX, 2, 2, 2, 2));
  ...
  SetWindowName("WCSim Event Display");
  MapSubwindows();
...
void WCSim_Draw::DoSave(){
  SaveAs(setno,eventno);
}

void WCSim_Draw::SaveAs(TString filename)
{
...
void WCSim_Draw::SaveAs(Int_t set,Int_t evt)
{
  TString filename;
...
WCSim_Draw::~WCSim_Draw()
{
   ...
   delete l1,l2,l3,l4;
   delete WCMain, WCTop, WCBottom, WCTop1, WCBottom1;
...
void WCSim_Draw::SetPalette(Int_t a){
  palette_Choice = a;
...
std::vector<Float_t> WCSim_Draw::ProjectToTankSurface_C(Float_t vtx[3],Float_t dir[3])
{
...
std::vector<std::vector<Float_t> > WCSim_Draw::FindRing(Float_t vertex[3],
						  Float_t dir[3],
...
void WCSim_Draw::DrawRing(Float_t vertex[3],Float_t dir[3],Int_t resolution,Float_t theta,Int_t color)
{
...
std::vector<Float_t> WCSim_Draw::ProjectToTankSurface_M(Float_t vtx[3],Float_t dir[3])
{
